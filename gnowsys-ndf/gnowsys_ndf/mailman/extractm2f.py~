...
#Imports from the configuration file
...
#			print new_thread
	...
	if re.search(r"^[A-Za-z0-9_.]+-owner$",name):
		...
		#If the user has subscribed/unsubscribed from a mailman mailing list, he/she must be added/removed from the corresponding Metastudio group.
		#If the user who has subscribed to a list is not found in the Metastudio database, the function sends an intimation to the user with a request to join Metastudio 	


		if re.search(r'^(.*) subscription notification$',subject):
			try:
				user = User.objects.get(email = user_address)
		...
		if re.search(r'^(.*) unsubscribe notification',subject):
			try:
				user_address = body.split(" ")[0]
