        ...
        for index, each_user in enumerate(group_users):
            try:
                ...
                try:
                    ...
                    temp_units_stat_str = analytics_data['units_progress_stmt']
                    each_row_dict['units_completed'] = int(temp_units_stat_str.split(' ')[0])
                    each_row_dict['total_units'] = int(temp_units_stat_str.split(' ')[3])
                    # analytics_data.pop('units_progress_stmt')

                    temp_modules_stat_str = analytics_data['module_progress_stmt']
                    each_row_dict['modules_completed'] = int(temp_modules_stat_str.split(' ')[0])
                    each_row_dict['total_modules'] = int(temp_modules_stat_str.split(' ')[3])
