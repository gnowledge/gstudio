...
grp_st = node_collection.one(
    {'$and': [{'_type': 'GSystemType'}, {'name': 'Group'}]})
...
def get_group_name_id(group_name_or_id, get_obj=False):
    ...
    if not get_obj:
        ...
        cache_key = 'get_group_name_id_' + \
            str(slug) if slug else str(abs(hash(group_name_or_id)))
...
def get_node_common_fields(request, node, group_id, node_type, coll_set=None):
    ...
    if content_org:
        if node.content_org != content_org:
            ...
            usrname = request.user.username
            filename = slugify(
                name) + "-" + slugify(usrname) + "-" + ObjectId().__str__()
            node.content = org2html(content_org, file_prefix=filename)
            is_changed = True
...
def create_task(task_dict, task_type_creation="single"):
    ...
    if task_type_creation == "group":
        ...
        if not task_dict.has_key("_id"):
            ...
            from_user = task_node.user_details_dict[
                "created_by"]  # creator of task

            group_name = node_collection.one(
                {'_type': {'$in': ["Group", "Author"]},
                    '_id': task_node.group_set[0]},
                {'name': 1}
            ).name

            url_link = "http://" + site + "/" + \
                group_name.replace(" ", "%20").encode(
                    'utf8') + "/task/" + str(task_node._id)
