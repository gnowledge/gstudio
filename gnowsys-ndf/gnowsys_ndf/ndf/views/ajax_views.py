...
def ajax_call(request, group_id):
	if request.method == "POST":
		#print "Values of check boxes \n"
		
		obj = request.POST.get("sVal",'')
		print "\noriginal :",obj,"\n"
		obj2 = list(removeArticles(str(obj)))
		print "\nword:",obj2,"\n"
		obj3 = list(stemWords(obj2))
		print "\nword stemmed:",obj3,"\n"
		#print "--------------------\n\n"

		regex=request.POST.get("sVal",'')
		regex=regex.lower()
		in_name=int(request.POST.get("name1",''))
		in_tags=int(request.POST.get("tags1",''))
		in_content=int(request.POST.get("contents1",''))
		by_author=request.POST.get("author1",'')
		...
		if (by_author != "None"):
			user = User.objects.get(username = by_author).pk
		else:
			user = "None"
		...
		objs = {'name':l1, 'tags':l2, 'content':l3, 'user':l4}
		"""
		objs_arr = {'exact':objs, 'split':objs2, 'stem':objs3}

		
		"""
		x = col.Node.find({"_type":"GSystemType"}, {"name":1, "_id":1})
		len1 = x.count()
		...
		if (in_name == 1):
			...
			for i in x:
				...
				# exact match
				h = col.Node.find({"member_of":i._id, "name":regex}, {"name":1, "_id":1})
				
				# split stemmed match
				g = []
				c = 0
				len_stemmed = len(obj3)
				while c < len_stemmed:
					g.append(col.Node.find({"member_of":i._id, "name":{"$regex":{obj3[c]}}, "$options":"i"}, {"name":1, "_id":1}))
					c += 1

				# like search
				f = col.Node.find({"member_of":i._id, "name":{"$regex":regex}}, {"name":1, "_id":1})
				"""
				for j in f:
					j[id2] = str(j[_id])
					del j[_id]
		...
		if (in_tags == 1):		
			...
			for i in x:
				...
				f = col.Node.find({"member_of":i._id, "tags":{"$regex":regex}}, {"name":1, "_id":1})
				"""
				for j in f:
					j[id2] = str(j[_id])
					del j[_id]
		...
		if (in_content == 1):
			...
			for i in x:
				...
				f = col.Node.find({"member_of":i._id, "content_org":{"$regex":regex}}, {"name":1, "_id":1})
		...
		if (by_author != "None"):
			...
			for i in x:
				...
				f = col.Node.find({"member_of":i._id, "created_by":user}, {"name":1, "_id":1})
				"""
				for j in f:
					j[id2] = str(j[_id])
					del j[_id]
		...
		return HttpResponse(json.dumps(objs, cls=Encoder))	
