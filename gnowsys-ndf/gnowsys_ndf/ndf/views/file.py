...
def file(request, group_id, file_id):
   ...
   * Renders a list of all 'Files' available within the database.
    ...
    if GST_FILE._id == ObjectId(file_id):
        ...
        files = collection.GSystem.find({'member_of': {'$all': [ObjectId(file_id)]}, '_type': 'File', 'group_set': {'$all': [group_id]}})
        imageCollection = collection.GSystem.find({'member_of': {'$all': [ObjectId(GST_IMAGE._id)]}, '_type': 'File', 'group_set': {'$all': [group_id]}})
        videoCollection = collection.GSystem.find({'member_of': {'$all': [ObjectId(GST_VIDEO._id)]}, '_type': 'File', 'group_set': {'$all': [group_id]}})
...
def uploadDoc(request, group_id):
    ...
    if  page_url:
        variable = RequestContext(request, {'page_url': page_url,'newgroup':group_id})
    else:
        variable = RequestContext(request, {'newgroup':group_id})
...
def save_file(files, title, userid, group_id, st_id, content_org, tags, access_policy, usrname):
    ...
    if fileobj.fs.files.exists({"md5":filemd5}):
        ...
        try:
            ...
            fileobj.group_set.append(group_id)        #group name stored in group_set field
...
def GetDoc(request, group_id):
    ...
    variable = RequestContext(request, {'filecollection':files,'newgroup':group_id})
    return render_to_response(template, variable)
...
def file_search(request, group_id):
    if request.method == "GET":
        ...
        variable = RequestContext(request, {'file_collection':file_search, 'view_name':'file_search','newgroup':group_id})
        return render_to_response(template, variable)
