...
def file(request, group_id, file_id):
   ...
   * Renders a list of all 'Files' available within the database.
    ...
    if GST_FILE._id == ObjectId(file_id):
        ...
        files = collection.GSystem.find({'gsystem_type': {'$all': [ObjectId(file_id)]},'_type':'File', 'group_set': {'$all': [group_id]}})
        already_uploaded = request.GET.getlist('var', "")
        return render_to_response("ndf/file.html", {'title': title, 'newgroup':group_id, 'files':files, 'already_uploaded':already_uploaded}, context_instance = RequestContext(request))
    else:
