...
if GSTUDIO_ELASTIC_SEARCH:
	...
	def search_detail(request,group_id,page_num=1):
		...
		if request.GET.get('field_list',None) == "attribute_type_set" or page_num > 1 :
			temp = node_collection.find({'_type': 'GSystem','access_policy':'PUBLIC','member_of': { '$nin': [ObjectId(group_id_of_twist)] } ,'group_set':ObjectId('55ab34ff81fccb4f1d806025')})
			...
			if_teaches = True
			paginator_search_result = paginator.Paginator(search_result, page_num, 15)
