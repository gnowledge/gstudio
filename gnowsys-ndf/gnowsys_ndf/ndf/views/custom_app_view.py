...
@login_required
def custom_app_create_edit(request, group_id, app_name, app_id, app_set_id=None, app_set_instance_id=None):
    ...
    app_set = collection.Node.one({'_id': ObjectId(app_set_id)}, {'name': 1})

    if app_set_instance_id:
        system = collection.Node.find_one({"_id": ObjectId(app_set_instance_id)})
        title =  app_set.name + ": " + system.name
    else:
        system = collection.GSystem()
        system.member_of.append(ObjectId(app_set_id))
        title =  app_set.name 

    system.get_neighbourhood(system.member_of)
    ...
    for l in property_display_order:
        print "\n l : ", l

    template = "ndf/custom_app_create_edit.html"

    variable = RequestContext(request, 
                              {'groupid': group_id, 'group_id': group_id, 'title': title,
                               'app_id': app_id, 'app_name':app_name, 'app_collection_set': app_collection_set, 'app_set_id': app_set_id,
                               'node': system, "property_display_order": property_display_order
                           })
