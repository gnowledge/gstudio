...
class RCS:
    ...
    def diff(self, name_rev, revision=None):
        """The output of rcsdiff
        
        """
        
        if revision == None:
            revCurrent = self.head(name_rev)
        else:
            revCurrent = revision
		
        splitVersion = revCurrent.split('.')
        previousSubNumber = int(splitVersion[1]) - 1

        if previousSubNumber <= 0:
            raise NameError, "No changes: New file"

        revPrevious = splitVersion[0] +"."+ str(previousSubNumber)

        # Diff
        f = self._open(name_rev, "rcsdiff -r"+ revPrevious +" -r"+ revCurrent) 
    ...
    def parseDiffOutput(self, stringArray):
        """
        - EXPERIMENTAL - Probably won't work the way you expect
        Returns a tuple of (rows that have changed, new rows, deleted rows).
        The new rows and deleted rows are string arrays. The rows that have
        changed is an array of tuples (old string, new string)
        """
        ...
        for line in stringArray:
            ...
            if line.startswith("---"):
                ...
                if replaceMode and addedLine != "" and removedLine != "":
                    replaced.append( (addedLine, removedLine) )
                else:
                    if addedLine != "":
